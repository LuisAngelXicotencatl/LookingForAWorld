...
<section>
	<div class="table-br">
       <h1 id="reviews" class="news">NINTENDO</h1>
       <?php
       include('../CONTROLLER/conexionbd.php');
       $juegos = "SELECT `plataformas`.`id_plataforma`, `plataformas`.`Id_juego`, `plataformas`.`xbox`, `juegos`.`nombre`, `juegos`.`portada`, `juegos`.`descripcionCorta`, `juegos`.`categoria`, `juegos`.`descripcion`, `juegos`.`Idiomas`, `juegos`.`link`, `juegos`.`linkPago` 
       FROM `plataformas` 
       LEFT JOIN `juegos` ON `plataformas`.`Id_juego` = `juegos`.`id_juego` 
       WHERE nintendo = 'Disponible'";
